...
from pycaret.clustering import load_model, predict_model, assign_model, create_model, setup, plot_model
import plotly.express as px  # type: ignore
from pycaret.clustering import setup, create_model, assign_model, plot_model, predict_model
...
st.header('Wytrenuj model samemu!')

value = st.slider('Wybierz ilość klastrów', 2, 10, 3)
if st.button('Trenuj'):
    #Trenowanie modelu
    s = setup(all_df, session_id=123)
    kmeans = create_model('kmeans', num_clusters=value)
    df_with_clusters = assign_model(kmeans)

    #Wyświetlanie liczby klastrów
    st.write('Dystrybucja osób w klastrach:', value)
    st.dataframe(df_with_clusters["Cluster"].value_counts())

    #Wyświetlanie wykresu dla klastrów
